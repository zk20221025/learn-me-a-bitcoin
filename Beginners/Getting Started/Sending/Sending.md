...
每个节点都会定期更新它们的区块链以包含最新的交易信息。这些交易现在已经得到确认。
...
1. **防止冲突的交易被写入区块链**。如果两个冲突的交易被发送到网络中（例如尝试将相同的比特币发送到两个不同的地方），由于挖掘
，只有其中一个交易实际上会被写入区块链。
...
* 如果内存池中的交易数量**超过**候选块的容量，则你正在与内存池中的其他交易竞争进入该区块（因此手续费金额将根据你想要的交易速度而变化）。
