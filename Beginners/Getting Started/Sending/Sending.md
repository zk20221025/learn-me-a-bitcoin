...
>**注意**：当你发送交易时，你的钱包会给你一个TXID（交易ID），这就像是交易的唯一参考号码。你可以使用它在[区块链浏览器](https://learnmeabitcoin.com/explorer)上查看交易的状态。

## 2. 进行交易时会发生什么？
当你进行比特币交易时，你的钱包会将你的交易信息发送到**比特币网络**中的一个节点。然后由这个节点将交易信息向其他节点传递，直到网络上的每个节点都拥有你的交易副本。  
...
每个节点都会定期更新它们的区块链来包含最新的交易信息。这些交易现在已经得到确认。

![Sending-5.png](img/Sending-5%20(1).png)

<center>区块链是比特币交易的永久存储。</center>

最后，这个过程会不停地重复，以便新的交易块以10分钟的间隔定期被添加到网络上的区块链中。
...
<center>矿工必须努力尝试将新的区块添加到区块链上。</center>

>**提示**：网络上的任何节点都可以成为矿工。但如果你只想成为一个节点（而不挖掘区块）也没问题。

**挖矿**过程是一个**全网范围的竞争**，网络上的任何节点都有机会挖掘下一个区块。算力越多的节点更有机会挖掘下一个区块，但挖矿的过程是**不可预测的**，因此没有任何一个节点可以控制挖到新的区块并添加到区块链中。

![Sending-7.png](img/Sending-7%20(1).png)

<center>任何都有机会节点挖掘出下一个区块。</center>
...
1. **防止冲突的交易被写入区块链**。如果两个冲突的交易被发送到网络中（例如尝试将相同的比特币发送到两个不同的地方），由于挖矿机制的存在，只有其中一个交易会被写入区块链。  
...
比特币交易可以包含一笔比特币作为手续费。这个手续费将被矿工收取，并作为矿工将你的交易包括在区块中的**激励**。
...
因此，当内存池中交易很多时，交易支付的手续费越高，该交易被包含在区块中的有效“优先级”就越高。
...
**应该在交易中放多少手续费？**  
这取决于**当前内存池中的交易数量**。
>内存池大小:    50238 transactions (56.89 MB)  
候选区块:  2932 transactions (1.51 MB)
...
>**注意**：交易按**sat/byte**排序，这是手续费（以satoshis为单位）除以交易大小（以字节为单位）。这是因为矿工希望获得每个交易在区块中占用的空间的最大手续费。

**如果在交易中设置的手续费太低会发生什么？**

如果你在交易上设置的手续费太低，你的交易将会在交易池中等待更长的时间才能被处理。
...
**提示**：如果你的交易在72小时内未被挖掘，你可以将其重新发送到网络中，以获得再次挖掘的机会。
...
等待**6次确认**（或更多）可以确保交易无法被撤销，因为此时矿工无法替换该数量的区块。然而，除非你正在保护自己免受恶意矿工对整个网络进行攻击的影响，否则这是过度保护。
