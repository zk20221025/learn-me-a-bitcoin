...
## 什么是难度？
**难度是一个数字，用于调节矿工将新的交易[块](../2.Blocks/Blocks.md)添加到[区块链](../1.Blockchain/Blockchain.md)中所需的时间。**

![difficulty-1.png](img/difficulty-1%20(1).png)

这个难度值每两周更新一次，以确保将新区块添加到区块链中平均需要10分钟。

## 为什么难度很重要？

>**因为它确保即使有更多的挖矿者加入网络，交易区块也会定期添加到区块链中，。**

如果难度保持不变，随着新的矿工加入网络，添加新区块到区块链之间的时间将缩短。
...
即使它包含字母，它仍然是一个数字。因此目标是一个十六进制值，而矿工们正在努力获得低于目标值的**十六进制哈希值**。
...
所以你通常会看到哈希值和目标值以一堆数字和字母的形式出现 - 它们是十六进制的，而不是人们更熟悉的十进制。但请记住，这两种十进制和十六进制数字具有相同的**值**，并且你可以轻松地在两者之间进行转换。
