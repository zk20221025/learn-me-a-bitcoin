# <center>难度</center>
<center>用于调节挖掘区块所需时间的机制。</center>

## 什么是难度？
**难度是一个数字，用于调节矿工将新的交易[区块](../2.Blocks/Blocks.md)添加到[区块链](../1.Blockchain/Blockchain.md)中所需的时间。**

![difficulty-1.png](img/difficulty-1%20(1).png)

这个难度值每两周更新一次，以确保将新区块添加到区块链的时间平均为10分钟。

## 为什么难度很重要？

>**因为它确保即使有更多的挖矿者加入网络，交易区块也会定期添加到区块链中，。**

如果难度保持不变，随着新的矿工加入网络，添加新区块到区块链的时间将缩短。

## 难度什么时候改变?
**难度调整发生在每2016个区块（大约每2周）。**

在此间隔期内，每个节点将预期挖掘时间（2016 x 10分钟）除以实际用时的时间（无论多少分钟）：
```
expected / actual
20160 / actual
```

如果矿工能够比预期更快地解决每个区块，比如每个区块只需要9分钟，那么你会得到这样的数字。
```
20160 / 18144 = 1.11
```
...
* 如果数字大于1（即挖掘的区块比预期更快），则难度增加。
* 如果数字小于1（即挖掘的区块比预期更慢），则难度降低。

比特币网络上的每个矿工现在都将使用这个新的难度来挖掘接下来的2016个区块。

>难度调整最多只会增加4倍（即数字不大于4或不小于0.25）。这是为了防止挖矿难度从一个级别突然跳跃到另一个级别，避免过度的波动。
...
现在，你可以每分钟随机生成一个1到100之间的数字。**你需要生成一个低于目标值的数字**。

比如说，如果设置目标值为**50**：

![difficulty-3.png](img/difficulty-3%20(1).png)

考虑到你每分钟只能生成一个1到100之间的数字，这应该需要**2分钟**时间。

但这太容易了。现在将目标降低到**20**，这意味着你只能有1/5的几率生成一个获胜数字，或者每**5分钟**才能生成一次：

![difficulty-4.png](img/difficulty-4%20(1).png)

目标越低，生成获胜号码就越困难。

>每次都不一定需要5分钟，因为你可能会在第一个生成的数字就得到幸运数字。但是长期来看，间隔时间将是5分钟。

因此，根据你每分钟能够生成多少数字，我可以使用目标高度来控制你找到获胜数字所需的时间。

**引入难度…**  
我是一台计算机，我通过**使用一个新的数字来划分一个数值范围**从而得到目标值，而不是直接告诉你目标值。

![difficulty-5.png](img/difficulty-5%20(1).png)

这个新的数字能够控制目标的高度。

**这个新数字就是难度，它是修改目标高度的简单方式。**
...
此外，我可以使用这个难度值来帮助我设定任何我想要的目标值：

![difficulty-6.png](img/difficulty-6%20(1).png)

难度越高，目标就越低。

因此使用难度来控制目标，以此来控制生成中奖号码所需的时间。

## 2. 比特币示例。
比特币的挖矿难度设置的原理和使用方式与此相同——它用于设定一个目标值，而矿工不断产生数字（对候选[区块](../2.Blocks/Blocks.md)进行哈希处理）来寻找一个低于这个目标值的数字。

![difficulty-7.png](img/difficulty-7%20(1).png)

由于矿工能够每分钟生成数千个数字（哈希值），因此比特币使用的数字极其巨大。
...
难度数字很大，所以目标极难达成。这就像中彩票一样难。

### 介绍十六进制数...

由于这些目标数字非常大，计算机更喜欢以十六进制格式来处理它们。
...
即使其中包含了字母，它仍然是一个数字。因此目标是一个十六进制值，而矿工们正在努力获得低于目标值的**十六进制哈希值**。

你可以很容易地在十六进制和“普通数字”（更为人所知的十进制数字）之间进行转换：
...
所以你通常会看到哈希值和目标值以一堆数字和字母的形式出现 - 它们是十六进制的，而不是人们更熟悉的十进制。但请记住，这两种十进制和十六进制数字表示的**值**是相同的，并且你可以轻松地在两者之间进行转换。

>虽然难度通常以十进制格式给出，而哈希和目标以十六进制存储，但是它们都是数字，只要将它们转换为相同的格式，你仍然可以进行相应的运算和处理。

![difficulty-11.png](img/difficulty-11%20(1).png)  

这是区块100,000的区块头信息。目标值是十六进制的，但它以紧凑格式存储在区块头（称为位）中。

**例子：利用难度找到目标。**
使用难度值计算出[第100,000个区块](http://blockexplorer.com/b/100000)的目标值。将其转换为十进制数，因为这样更容易理解。

以下是第100,000个区块的难度值（如区块头所示）：
```
{
...
```
>我使用了这个[十六进制转十进制的转换器](https://learnmeabitcoin.com/tools/hexdec)来做到这一点。
...
当矿工尝试解决第100,000个区块时，她想要为候选区块获取一个哈希值，该哈希值应该低于**1861311315012765306929610463010191006516769515973403833769533170**。
...
```
是的，这个哈希值比目标值低，所以是成功的，该区块可以添加到区块链中。

>目标哈希和区块哈希都将作为十六进制数字存储在区块头中，因此，如果我们将它们都转换回十六进制，它们将如下所示：
```
...
### 在哪里可以找到当前的难度？
你可以通过在比特币客户端中输入**getdifficulty**命令来查找当前的难度：

>难度也可以在**getmininginfo**（以及其他挖矿信息）中找到。
