...
首先，[挖矿](../Mining.md)是不可预测的，因此您永远不知道矿工何时会找到下一个区块哈希小于当前目标的区块。
...
此外，所有节点持续共享区块链的相同视图（因为它们始终采用[可用的最长区块链](../../Blockchain/longest-chain/longest-chain.md)）。因此，随着每个新块的挖掘，节点也将持续计算相同的目标值。
...
这是获取当前目标的最简单方法。当您请求一个[挖掘](../Mining.md)的区块模板时，它也会返回当前目标：
```
...
同样，您可以询问当前的[难度](../../../Beginners/How%20Bitcoin%20Works/2.Mining/3.Difficulty/Difficulty.md)，然后将其转换为目标难度：
```
$ bitcoin-cli getdifficulty
49549703178593
```
...
另外，正如之前提到的，在挖掘时的目标存储在每个[块头](../../Block/block-header/block-header.md)中的[比特](../../Block/block-header/bits/bits.md)格式中（您可以将其转换为完整的目标值）。
...
>**官方目标**：官方目标是存储在块头中的截断目标（在[比特](../../Block/block-header/bits/bits.md)字段中），因此它实际上不是调整前一个目标后获得的完整精度目标。
