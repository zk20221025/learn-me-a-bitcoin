...
>**注意**：换句话说，[钱包](../../../Beginners/Getting%20Started/getting-started/getting%20started.md)需要使用一个大的随机数来生成所有的密钥和地址，而助记句为我们提供了一种方便的方式来存储这个数字。
...
你可以将熵视为一个非常大的随机数，没有人能在以前或以后生成它。最好将此数字视为一系列位（例如10011010010001…），这是计算机存储数字的方式。
...
首先，为熵添加校验和以帮助检测错误（使最终句子对用户更友好）。这个校验和是通过SHA256对熵进行哈希创建的，这为我们的熵提供了唯一的指纹。然后取该哈希的1位，用于每32位熵，并将其添加到熵的末尾。
