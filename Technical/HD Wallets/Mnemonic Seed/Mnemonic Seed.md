...
**助记词句子**（“助记码”，“种子短语”，“种子词语”）是一种将大的随机数转化为一系列[单词](#bip39单词列表)的方式，使人们更容易记住和存储。

然后使用这些单词创建**种子**，可用于在[分层确定性钱包](../HD%20Wallets.md)中生成[扩展密钥](../Extended%20Keys/Extended%20Keys.md)。

>**注意**：[钱包](../../../Beginners/Getting%20Started/getting-started/getting%20started.md)需要使用一个大的随机数来生成所有的密钥和地址，而助记词句子为我们提供了一种方便的方式来存储这个数字。

>**不要相信在网站上生成的种子**。创建钱包时，应在自己的计算机上生成自己的种子。我建议使用 [Electrum](https://electrum.org/)。
...
要创建助记词，首先需要生成熵，这是我们的随机性的来源。

你可以将熵视为一个非常大的随机数，没有人能在以前或以后生成它。最好将此数字视为一系列的位（例如**10011010010001…**），这是计算机存储数字的方式。

>**提示**：位（**0或1**）是计算机上最小的存储单位。

我们生成的熵必须是32位的倍数，这样才能将它平均分成偶数块并在之后转换为单词。此外，熵应该**介于128到256位之间**，因为这样才能确保两个人不可能生成相同的熵。
```ruby
# -------------------
# 1. Generate Entropy
# -------------------
...
```
>**注意**：始终使用安全的随机数生成器来生成熵。不要使用编程语言的默认“随机”函数，因为它产生的数字对于密码学来说不够随机，不能提供足够的安全性。[^1]

### 2. 熵转换为助记词
现在已经有了熵，可以将其编码成单词以生成助记词。

首先，为熵添加校验和以帮助检测错误（使最终句子更方便使用）。这个校验和是通过SHA256对熵进行哈希创建的，这为我们的熵提供了唯一的指纹。然后取该哈希的每32位熵的1位，并将其添加到熵的末尾。

![Mnemonic Seed-2.png](img/Mnemonic%20Seed-2%20(1).png)

接下来，将带校验和的熵分成分成11位一组，将这些数字转换为十进制数，并使用这些数字来选择相应的单词。
...
>**提示**：11位数字可以存储0-2047之间的小数（这就是为什么[单词列表](#bip39单词列表)中有2048个单词的原因）。

>**提示**：通过在每32位熵中添加1位校验和，将始终得到33位的倍数，可以将其分成相等的11位的块。

>**注意**：助记词的长度通常为**12到24个单词之间**。
```
# ----------------------
...
### 3. 从助记词转换为种子
现在有了助记词句子，可以将其转换成最终的种子。

要创建种子，需要将助记词句子通过 PBKDF2 函数（基于密码的密钥派生函数2）。这将对你的助记词（+ 可选密码）多次哈希，直到产生最终的 64 字节（512 位）结果。

![Mnemonic Seed-4.png](img/Mnemonic%20Seed-4%20(1).gif)

可选密码允许你修改最终种子。

这个64字节的结果就是你的**种子**，可以用来创建分层确定性钱包中的[主扩展密钥](../Extended%20Keys/Extended%20Keys.md)。
```ruby
# -------------------
# 3. Mnemonic to Seed
# -------------------
...
PBKDF2是一个哈希函数，旨在通过多次哈希数据来减慢计算速度，从而使得人们更难对助记语句进行暴力破解来获取实际使用的种子。  
此外，PBKDF2还允许你提供第二个输入，称为salt（“密码”，“种子扩展”），以及你要哈希的数据，这使能够从相同的助记语句生成完全不同的种子。
...
助记词句子包含校验和，这意味着可以**检查给定的助记词句子是否有效**。

要验证助记词，你需要将助记词中的单词转换回位，确定熵部分和校验和部分，然后检查从熵部分创建的校验和是否与助记词中的校验和匹配。

![Mnemonic Seed-5.png](img/Mnemonic%20Seed-5%20(1).png)

助记符句子是使用熵加上校验和构建的。

>**警告：不要随机生成12-24个单词来创建助记词句子**。如果你只是随机选择12-24个单词，那么你最后选择的单词可能不包含其他单词的正确校验和，导致当你尝试导入到钱包时会显示无效。
```ruby
# -----------------
# Validate Mnemonic
# -----------------
...
### BIP39单词列表
助记词句子中的单词来自固定的2048个单词列表（由[BIP39](https://github.com/bitcoin/bips/blob/master/bip-0039/bip-0039-wordlists.md)指定）。每个单词的前**4个字母**在列表中是唯一的。
```
abandon
ability
able
