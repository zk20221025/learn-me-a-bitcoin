...
**助记词句子**（“助记码”，“种子短语”，“种子词语”）是一种将大量随机生成的数字表示为一系列[单词](#bip39单词列表)的方法，使人们更容易存储。

然后使用这些单词创建**种子**，可用于在[分层确定性钱包](../HD%20Wallets.md)中生成[扩展密钥](../Extended%20Keys/Extended%20Keys.md)。

>**注意**：换句话说，你的[钱包](../../../Beginners/Getting%20Started/getting-started/getting%20started.md)需要使用一个大的随机数来生成所有的密钥和地址，而助记句为我们提供了一种方便的存储方式。
...
我们生成的熵必须是32位的倍数，因为这将允许我们将熵分成偶数块并在以后将其转换为单词。此外，熵应该**介于128到256位之间**，因为这足以使两个人无法生成相同的熵。
```ruby
# -------------------
# 1. Generate Entropy
# -------------------
...
提示：通过在每32位熵中添加1位校验和，将始终得到33位的倍数，可以将其分成相等的11位块。
...
### 3. 从助记词到种子
现在有了助记词，可以将其转换成最终的种子。

要创建种子，需要通过 PBKDF2 函数输入助记词句子。这基本上是将你的助记词（+ 可选的口令）多次哈希，直到产生最终的 64 字节（512 位）结果。
...
PBKDF2基本上是一个哈希函数，旨在通过多次哈希数据来减慢计算速度，从而使得人们更困对助记语句进行暴力破解来获取实际使用的种子。  
