...
我们生成的熵必须是32位的倍数，这样才能将它平均分成块并在之后转换为单词。此外，熵应该**介于128到256位之间**，因为这足以使两个人不可能生成相同的熵。
```ruby
# -------------------
# 1. Generate Entropy
# -------------------
...
要创建种子，需要通过 PBKDF2 函数输入助记词句子。这将对你的助记词（+ 可选密码）多次哈希，直到产生最终的 64 字节（512 位）结果。
...
>[PBKDF2](https://en.wikipedia.org/wiki/PBKDF2) - **基于密码的密钥派生函数**  
PBKDF2是一个哈希函数，旨在通过多次哈希数据来减慢计算速度，从而使得人们更难对助记语句进行暴力破解来获取实际使用的种子。  
此外，PBKDF2还允许你提供第二个输入，称为盐（“密码”，“种子扩展”），以及你要哈希的数据，这使能够从相同的助记语句产生完全不同的种子。
...
### BIP39单词列表
助记词句子中的单词来自固定的2048个单词列表（由[BIP39](https://github.com/bitcoin/bips/blob/master/bip-0039/bip-0039-wordlists.md)指定）。每个单词的前4个字母在列表中是唯一的。
```
abandon
ability
able
