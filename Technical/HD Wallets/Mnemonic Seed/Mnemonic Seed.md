...
**助记词句子**（“助记码”，“种子短语”，“种子词语”）是一种将大的随机生成的数字转化为一系列[单词](#bip39单词列表)的方式，使人们更容易存储。
...
你可以将熵视为一个非常大的随机数，没有人能在以前或以后生成它。最好将此数字视为一系列位（例如**10011010010001…**），这是计算机存储数字的方式。

>**提示**：位（**0或1**）是计算机上最小的存储单位。

我们生成的熵必须是32位的倍数，这样才能将它平均分成偶数块并在之后转换为单词。此外，熵应该**介于128到256位之间**，因为这足以使两个人不可能生成相同的熵。
```ruby
# -------------------
# 1. Generate Entropy
# -------------------
...
```
>**注意**：始终使用安全的随机数生成器来生成熵。不要使用编程语言的默认“随机”函数，因为它产生的数字对于密码学来说不够随机。[^1]

### 2. 熵到助记词
现在已经有了熵，可以将其编码成单词。

首先，为熵添加校验和以帮助检测错误（使最终句子更方便使用）。这个校验和是通过SHA256对熵进行哈希创建的，这为我们的熵提供了唯一的指纹。然后每32位熵取该哈希的1位，并将其添加到熵的末尾。

![Mnemonic Seed-2.png](img/Mnemonic%20Seed-2%20(1).png)

接下来，将它分成11位一组，将这些数字转换为十进制数，并使用这些数字来选择相应的单词。
...
要创建种子，需要通过 PBKDF2 函数（基于密码的密钥派生函数2）输入助记词句子。这将对你的助记词（+ 可选密码）多次哈希，直到产生最终的 64 字节（512 位）结果。
...
此外，PBKDF2还允许你提供第二个输入，称为salt（“密码”，“种子扩展”），以及你要哈希的数据，这使能够从相同的助记语句产生完全不同的种子。
...
要验证助记词，你需要将助记词中的单词转换回位，确定熵部分和校验和部分，并检查从熵中创建的校验和是否与助记词中的校验和匹配。
