...
>你可以将HMAC视为**哈希函数**，允许你传递数据并附加密钥以生成一组新的随机字节。
...
### 扩展公钥
我们还可以创建相应的扩展公钥。这只需要计算私钥对应的公钥，并将其与同样的链码结合。
...
**新私钥**是HMAC结果的前32个字节，加上原始私钥。这只是将原始私钥增加了一个随机的32字节数字。我们对新**私钥**进行模运算，以保持新私钥在椭圆曲线的有效数字范围内。
...
>提示：这就是为什么强化子密钥很有用，因为在树的一个层级中丢失一个子私钥永远不会使其他子私钥面临被派生的风险。
