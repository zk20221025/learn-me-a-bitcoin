...
**扩展密钥**是[私钥](../../Keys/Private%20Key/Private%20Key.md)或[公钥](../../Keys/Public%20Key/Public%20Key.md)，可以在[分层确定性钱包](../HD%20Wallets.md)中使用它来派生新的密钥。

因此，你可以有一个**扩展私钥**，并将其用作钱包中所有子**私钥和公钥**的来源。此外，相应的**扩展公钥**将生成相同的**子公钥**。

## 1. 主扩展密钥
你的第一个扩展密钥（主密钥）是通过将[种子](../Mnemonic%20Seed/Mnemonic%20Seed.md)（64字节的熵）输入HMAC-SHA512哈希函数创建的。
...
HMAC函数返回**64字节**的数据（这是完全不可预测的）。我们将其分成两半来创建我们的**主扩展私钥**：
...
* **扩展私钥**可以生成具有新**私钥和公钥**的子密钥。
* **扩展公钥**只能生成具有新**公钥**的子密钥。
...
2. 强化-只有**扩展私钥**可以生成**公钥**。

索引**2147483648到4294967295**（所有可能的子密钥的后一半）

换句话说，强化子密钥为你提供了创建“秘密”或“内部”公钥的选项，因为扩展公钥无法派生它们。

## 3. 子扩展密钥派生
**扩展私钥和扩展公钥**都可以派生子密钥，每个子密钥都有自己独特的**索引**编号。
...
1. **计算出公钥**。（这样，相应的扩展公钥就可以将相同的数据放入HMAC函数中，以便生成它们的子级。）
...
**新私钥**是HMAC结果的前32个字节，加上原始私钥。这只是将原始私钥增加了一个随机的32字节数字。我们曲线的阶数（n = 115792089237316195423570985008687907852837564279074904382605163141518161494337）对新**私钥**进行模运算，以保持新私钥在椭圆曲线的有效数字范围内。

总而言之，使用父扩展私钥中的数据（**公钥**+**索引**，**链码**）并将其通过HMAC函数产生一些新的随机字节。然后使用这些新的随机字节来构造下一个私钥。
...
然而，这个强化的子密钥是通过将**私钥**放入HMAC函数中来构建的（扩展公钥无法访问），这意味着以这种方式派生的子扩展私钥将拥有一个无法由相应的扩展公钥派生的公钥。

>除非你需要生成没有私钥访问权限的公钥，否则强化派生应该是默认设置。– Pieter Wuille2[^2]
...
**安全提示**
由于**普通**子密钥的派生方式，如果你拥有**扩展公钥**和任何子**私钥**，则有可能计算出父**扩展私钥**。

![Extended Keys-13.png](img/Extended%20Keys-13%20(1).png)

换句话说，如果你的**扩展公钥**是公开的，一定不要泄露子**私钥**。如果你泄露了，任何人都可以倒推计算出**扩展私钥**，并从该层级中的**所有子密钥**中窃取比特币。

>**提示**：这就是为什么**强化**子密钥很有用，因为在树的一个层级中丢失一个子私钥永远不会使其他子私钥面临被派生的风险。
...
## 5. 序列化
扩展密钥可以被序列化以便于传递。这些序列化的数据包括**私钥/公钥和链码**，以及一些附加的元数据（描述其他数据的数据，充当信息标签）。
...
* [为什么强化的xpub密钥无法生成子公钥？](https://bitcoin.stackexchange.com/questions/92562/why-hardended-xpub-key-cannot-generate-child-public-key)
