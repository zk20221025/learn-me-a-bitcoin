...
因此，扩展私钥最终只是普通私钥与**链码**相结合。

![Extended Keys-3.png](img/Extended%20Keys-3%20(1).png)

### 扩展公钥
我们还可以创建相应的扩展公钥。这只需要计算私钥对应的公钥，并将其与同样的**链码**结合。
...
## 3. 子扩展密钥派生
扩展私钥和扩展公钥都可以派生子密钥，每个子密钥都有自己独特的**索引**编号。
...
**新链码**是HMAC结果的最后32个字节。这只是一组唯一的字节，可以用它来生成新的链码。
...
因此，生成子扩展私钥时将相同的数据和密钥放入HMAC函数中。然后，通过[椭圆曲线点加法](../../Keys/ECDSA/ECDSA.md#相加)，使用HMAC结果的前32字节（这意味着它对应于子扩展私钥中的私钥）来计算出子公钥。
...
**安全提示**
由于**普通**子密钥的派生方式，如果你拥有扩展公钥和任何子私钥，则有可能计算出父扩展私钥。

![Extended Keys-13.png](img/Extended%20Keys-13%20(1).png)

换句话说，如果你的扩展公钥是公开的，一定不要泄露子私钥。如果你泄露了，任何人都可以倒推计算出扩展私钥，并从该层级中的**所有子密钥**中窃取比特币。
...
## 5. 序列化
扩展密钥可以被序列化以便于传递。这些序列化的数据包括私钥/公钥和**链码**，以及一些附加的元数据（描述其他数据的数据，充当信息标签）。
...
>在密钥字段中，私钥（32字节）以**0x00**开头，以将其扩展到与公钥（33字节）相同的长度。
