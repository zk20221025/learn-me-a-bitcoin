...
**扩展密钥**是一种特殊的[私钥](../../Keys/Private%20Key/Private%20Key.md)或[公钥](../../Keys/Public%20Key/Public%20Key.md)，可以在[分层确定性钱包](../HD%20Wallets.md)中使用它来派生新的密钥。

你只需要一个**扩展私钥**，就可以生成你钱包中的所有子**私钥和公钥**。此外，相应的**扩展公钥**将生成相同的**子公钥**。这种方式提供了一种更为安全和高效的管理钱包密钥的方法。

## 1. 主扩展密钥
你的第一个扩展密钥（主密钥）是通过将[种子](../Mnemonic%20Seed/Mnemonic%20Seed.md)（64字节的熵）输入HMAC-SHA512哈希函数生成的。

>你可以将HMAC（基于哈希的消息验证码）视为**哈希函数**，它允许你将数据和附加密钥一起传递以生成一组新的随机字节。

![Extended Keys-2.gif](img/Extended%20Keys-2%20(1).gif)

在创建主扩展密钥时，实际上不需要使用密钥，因此只使用任意字符串“Bitcoin seed”。[^1]

HMAC函数会返回**64字节**的数据（这是完全不可预测的）。我们将其分成两半来创建我们的**主扩展私钥**：

* 左半部分将是**私钥**，就像任何其他私钥一样。
* 右半部分将是**链码**，只是额外的32个字节的随机数据。
  
>生成子密钥需要链码。如果你获得了私钥但**没有**链码，将无法派生出后代密钥（从而保护它们）。

### 扩展私钥

**扩展私钥**只是**普通私钥**与**链码**相结合。

![Extended Keys-3.png](img/Extended%20Keys-3%20(1).png)

### 扩展公钥
我们还可以创建相应的**扩展公钥**。这只需要计算**私钥**对应的**公钥**，并将其与同样的**链码**结合。

![Extended Keys-4.png](img/Extended%20Keys-4%20(1).png)

这样，我们就得到了初始的**主扩展私钥**和**主扩展公钥**。
...
每个子密钥也有一个索引编号（最多**2 ** 32**）。

![Extended Keys-5.png](img/Extended%20Keys-5%20(1).png)

扩展公钥的特点在于它们可以生成与扩展私钥相同的公钥。

为了安全起见，你可以从**扩展私钥**派生出两种类型的子密钥：

1. 普通-**扩展私钥和扩展公钥**可以生成相同的**公钥**。

索引**0到2147483647**（所有可能的子密钥的前一半）

2. 加固-只有**扩展私钥**可以生成**公钥**。

索引**2147483648到4294967295**（所有可能的子密钥的后一半）

加固子密钥让你有可能创建一个扩展公钥无法生成的“秘密”或“内部”公钥。

## 3. 子扩展密钥派生
**扩展私钥和扩展公钥**都可以派生子密钥，每个子密钥都有自己唯一的**索引**编号。
...
2. 加固**子扩展私钥**  
3. 普通**子扩展公钥**  
>提示：派生的子扩展密钥（和父密钥）彼此独立。换句话说，你无法从一个子密钥推断出它与扩展树中的其他密钥是否有任何联系。
...
1. **计算出公钥**。（以便在派生子密钥时，对应的扩展公钥可以将相同的数据输入HMAC函数。）
2. **使用介于0和2147483647之间的索引**。这个范围内的索引被指定为普通子扩展密钥。
3. **通过HMAC将数据和密钥放入**。
   * 数据=**公钥**+**索引**（连接）
   * 密钥=**链码**

**新链码**是HMAC结果的最后32个字节。这是一组唯一的字节，可以用它来生成新的链码。

**新私钥**是HMAC结果的前32个字节，加上原始私钥。这只是将原始私钥增加了一个随机的32字节数字。我们曲线的阶数（n = 115792089237316195423570985008687907852837564279074904382605163141518161494337）对新**私钥**进行模运算，以保证新私钥在椭圆曲线的有效数字范围内。

总而言之，使用父扩展私钥中的数据（**公钥**+**索引**，**链码**）并将其通过HMAC函数产生一些新的随机字节。然后使用这些新的随机字节从旧的私钥构造新的私钥。
...
1. **使用介于2147483647和4294967295之间的索引**。这些索引被指定为加固子扩展密钥。
2. **通过HMAC将数据和密钥传递。**
   * 数据=**私钥**+**索引**（连接）
   * 密钥=**链码**
...
**新的私钥**是 HMAC 结果添加到原始私钥的前 32 个字节。这同样是将原始私钥增加一个随机的32字节数。

然而，这个加固子密钥的生成方式是通过将**私钥**放入HMAC函数（扩展公钥无法访问），这意味着以这种方式派生的子扩展私钥将拥有一个无法由相应的扩展公钥派生的公钥。

>除非你需要生成没有私钥访问权限的公钥，否则加固派生应该是默认设置。– Pieter Wuille2[^2]
...
1. **使用0到2147483647之间的索引**。这些索引用于普通子扩展私钥。
2. **将数据和密钥通过HMAC进行处理。**
   * 数据=**公钥**+**索引**（连接）
   * 密钥=链代码

**新链码**是HMAC结果的最后32个字节。这将是与上面的普通子扩展私钥相同的链代码，因为如果你回顾一下，就会发现我们将相同的输入放入HMAC函数中。

**新的公钥**是原始公钥点加上HMAC结果的前32个字节作为曲线上的点（乘以生成器得到的点）。

因此，生成子扩展私钥时将相同的数据和密钥放入HMAC函数中。然后，通过[椭圆曲线点加法](../../Keys/ECDSA/ECDSA.md#相加)，使用HMAC结果的前32字节（这意味着它对应于**子扩展私钥**中的**私钥**）来计算出子**公钥**。
...
换句话说，从**扩展公钥**派生的**公钥**如何与从**扩展私钥**派生的**私钥**相对应？

对于两个子扩展密钥，将相同的输入放入HMAC函数中，因此得到相同的数据作为结果。然后使用这些数据的前32个字节（是一个数字）：

* 通过这个数字将父**私钥**增加，以创建子**私钥**。

* 通过相同的数量将父**公钥**增加，以创建子**公钥**。

由于椭圆曲线数学的工作方式，子**私钥**将对应于子**公钥**。
...
**安全提示**
由于**普通**子密钥的派生方式，如果你拥有一个**扩展公钥**和任何一个子**私钥**，则有可能计算出父**扩展私钥**。

![Extended Keys-13.png](img/Extended%20Keys-13%20(1).png)

换句话说，如果你的**扩展公钥**是公开的，一定不要泄露子**私钥**。否则任何人都可以倒推计算出**扩展私钥**，进而窃取该层级中的**所有子密钥**对应的比特币。

>**提示**：这就是为什么**加固**子密钥很有用，因为在树的一个层级中丢失一个子私钥也不会让其他子私钥有被派生的风险。
...
## 5. 序列化
扩展密钥可以被序列化，使其更便于传输和使用。这些序列化的数据包括**私钥/公钥和链码**，以及一些其他的元数据（描述其他数据的数据，充当信息标签）。
...
>在密钥字段中，**私钥**（32字节）以**0x00**开头，以将其扩展到与**公钥**（33字节）相同的长度。

然后在此数据中添加[校验和](../../Keys/Checksum/Checksum.md)（以帮助检测错误输入），最后将所有内容转换为[Base58](../../Keys/Base58/Base58.md)（这样生成的扩展密钥格式更便于人类阅读和理解）。
...
```
正如你所看到的，它们非常长，这是因为它们包含了关于扩展密钥的额外的有用的信息。
...
* [为什么加固的xpub密钥无法生成子公钥？](https://bitcoin.stackexchange.com/questions/92562/why-hardended-xpub-key-cannot-generate-child-public-key)
