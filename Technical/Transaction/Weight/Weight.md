...
这种权重测量给交易的验证部分（签名数据）提供了折扣，使其占用的空间相对较少。换句话说，**签名数据的成本低于交易数据的其余部分**。
...
使用旧的度量标准，每增加一字节的交易数据，其[交易手续费](../Fees/Fees.md)就会增加。因此，花费一个输出的成本比创建一个输出的成本更高。
...
因此，这为花费输出创造了更加平衡的激励，这有助于减少未花费交易输出[UTXO](../UTXO/UTXO.md)集的大小（UTXO集在运行完整节点的计算机上RAM中占用宝贵的空间）。
