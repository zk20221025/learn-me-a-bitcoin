...
这种权重测量给交易的验证部分（签名数据）提供了折扣，使其在区块中占用的空间减少。这就意味着，**签名数据的成本低于交易数据的其余部分**。
...
在传统的[交易数据](../Transaction%20Data/Transaction%20Data.md)中，可以使用约**32字节**创建输出，但花费它需要约**108字节**：

![Weight-2.png](img/Weight-2%20(1).png)

使用旧的度量标准，交易中包含的每一个字节的数据都会增加[交易手续费](../Fees/Fees.md)。因此，花费一个输出的成本比创建一个输出的成本更高。

然而，新的权重指标使得创建和花费一个输出的成本更为平衡。

![Weight-3.png](img/Weight-3%20(1).png)

通过使用新的权重指标，花费一项产出与创建一项产出的“成本”大致相同。

这种设计形成了一个更平衡的激励机制来花费输出，这有助于减少未花费交易输出[UTXO](../UTXO/UTXO.md)集的大小（UTXO集在运行完整节点的计算机上RAM中占用宝贵的空间）。
