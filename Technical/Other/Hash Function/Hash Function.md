...
加密哈希函数产生的结果是随机的（没有规律），因此无法通过反向计算哈希函数来确定原始数据是什么。
...
### 1. Hash256
这涉及将数据通过[SHA-256](https://github.com/in3rsha/sha256-animation/)哈希函数进行哈希处理得到一个哈希值，然后再将这个哈希值再次放入SHA-256函数，得到最后的哈希值。这就是“双SHA-256”。我们简称为Hash256。
...
它仅在创建传统[地址](../../Keys/Address/Address.md)（例如以1或3开头的地址）的过程中缩短[公钥](../../Keys/Public%20Key/Public%20Key.md)和[脚本](../../Script/Script.md)时使用。它尚未用于任何需要对比特币数据进行哈希处理的最新开发中。
...
>**最终，所有比特币数据都只是一堆字节。我们只是以十六进制字符串的表示形式来方便使用。**
