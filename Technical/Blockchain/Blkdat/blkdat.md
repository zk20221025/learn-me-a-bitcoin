...
此外，它们不是将整个区块链存储在一个巨大的文件中，而是分成多个blk * .dat文件。
...
节点首先将区块添加到**blk00000.dat**中，当它填满后，它会继续移动到**blk00001.dat**，然后是**blk00002.dat**……以此类推。

### 例子

blk.dat文件中的数据以**二进制**形式存储，每个新区块都会附加到文件末尾。

可以通过读取**blk00000.dat**的前293个字节来查看**创世块**。
```
...
* **魔术字节**和**size**字段可以帮助你确定每个块的数据开始和结束的位置。
* **区块头**。
* **交易数量**（[可变整数](../../Other/VarInt/varint.md)），随后是**每个交易的交易数据**。
...
```
>该**size**字段能让人知道需要读取293字节才能获取整个区块。  
整个size字段值为**1d010000**，然后将其转换成人类可以理解的格式：  
1.[交换字节顺序](https://learnmeabitcoin.com/tools/swapendian)以获得**0000011d**  
2.[转换为十进制](https://learnmeabitcoin.com/tools/hexdec)以获得**285**  
因此，除了初始的8个字节用于**魔术字节 + size**字段外，接下来的区块数据的大小将是**285个字节**。
...
>这是因为比特币节点会并行下载区块，以便尽快完成区块链的下载。节点不会等待按照顺序接收每个区块，而是会提前下载后续的区块。  
你的节点提前下载的最大范围（（或者说“最大的无序度”）由比特币源代码[BLOCK_DOWNLOAD_WINDOW](https://github.com/bitcoin/bitcoin/search?q=BLOCK_DOWNLOAD_WINDOW)控制。
...
### Linux工具
如上所述，blk.dat文件中的数据是以二进制形式存储的，如果你直接用文本编辑器打开，可能无法理解其中的内容。但是没有关系，因为二进制数据可以轻松转换为[十六进制](../../Other/Hexadecimal/hexadecimal.md)，这可以通过两个命令来完成此任务：
...
0000125
```
* **-C** <- 在显示数据时，使用与比特币相同的字节顺序，并且也显示ASCII文本
* **-s** <- 起始点（以字节为单位的偏移量）
* **-n** <- 要读取的字节数

>**仅显示十六进制数据。**
你可以通过组合一些命令，只获取原始的十六进制数据输出：
```
hexdump -C -s 8 -n 285 blk00000.dat | cut -c 11-58 | tr '\n' ' ' | tr -d ' '
```
* **cut -c 11-58** <- 剪切出每行的第11个字符到第58个字符之间的所有内容
* **tr '\n' ' '** <- 将换行符替换为空格
* **tr -d ' '** <- 删除所有空格
