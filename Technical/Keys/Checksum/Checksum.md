...
## 它们是如何工作的？
在比特币中，校验和是通过两次通过*SHA256*[哈希数据](../../Other/Hash%20Function/Hash%20Function.md)并取结果的前4个字节来创建的：
...
为此，您首先需要从[base58](../Base58/Base58.md)解码地址。然后，从**校验和中分离出数据部分**，并验证从数据中计算出的校验和是否与给定的校验和相匹配。
```
...
```
>一个**Base58解码的地址**包含：一个前缀，某个东西的hash160（例如[公钥哈希](../Public%20Key/Public%20Key%20Hash/public-key-hash.md)）和一个校验和。但是在这里你真正需要知道的是校验和是最后4个字节。
...
校验和是一个随机的4字节[十六进制数](../../Other/Hexadecimal/hexadecimal.md)，因此发生这种情况的概率是**0xFFFFFFFF**中的1。用十进制表示，这是**4,294,967,295中的1**。
