...
Base58是一组字符集，可以用来将大数字转化成更短、更便于用户理解和使用的格式。
...
在日常生活中，我们习惯于使用**十进制**（即使用数字**0-9**）。

但是对于计算机来说，它可以很容易地使用更多的字符来表示更大或者更复杂的值，这是因为计算机可以轻松处理更大范围的数值和更复杂的数值系统。
...
所有这些“数字”都具有相同的数值——**它们只是使用不同的字符集（基数）来表示它。**

>基数中的字符越多，表示大数值所需要字符就越少。因此，基数越大，“数字”分表示形式越短。

## 为什么是base58？

base58编码使用了字母数字字母表中的58个字符（去除了容易混淆的**0、O、l和I**）
...
将整数（十进制）转换为Base58，需要使用**取模运算**[^1]函数。

你需要不断地将你的十进制数除以58，并在每一步中取出**余数**。这个余数就是Base58中对应的**字符索引**。你需要重复这个过程，直到所有的余数都被取出，这样你就完成了Base58的编码。
```
base10 = 123456789
...
要将Base58值转换为十进制，需要将每个**字符的索引**乘以数字中该位置表示的**58数量**。
...
1. [WIF私钥](../Private%20Key/WIF%20Private%20Key/WIF%20Private%20Key.md)
   * [私钥](../Private%20Key/Private%20Key.md)就像“主密码”，用于导入比特币到新的钱包。这个私钥通常用Base58编码表示，称为WIF私钥。

2. [地址](../Address/Address.md)
   * [公钥](../Public%20Key/Public%20Key.md)是私钥的公开对应部分，用于发送比特币。但公钥通常很长，不便于使用，因此我们通常将其转换为更短的地址。在这个转换过程的最后一步，我们会使用Base58编码。
...
数字开头的零不会改变它的大小（例如**0x12与0x0012**是相同的），因此当我们转换为base58（它使用取模运算函数）时，任何额外的零在开头将不会影响结果。
...
>如前所述，十六进制前缀**00**在编码为base58时不会自动转换为“1”。这种转换是在代码中手动执行的。
...
这些代码片段执行的是**比特币中使用的base58转换**。它们可以在十六进制和base58之间转换，因为十六进制是我们最常使用的数据格式。
>### Ruby:
```ruby
module Base58
