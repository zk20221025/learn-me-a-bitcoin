...
你知道，在NULL DATA脚本出现之前，人们通过使用现有的标准锁定脚本来向交易中添加数据。例如，你可以始终使用[P2PKH](../P2PKH/P2PKH.md)脚本，将任意数据放在**散列公钥**应该放置的位置：
...
>>关于OP_RETURN：在0.9版本中，关于OP_RETURN功能和区块链中的数据存在一些混淆和误解。这个改变并不是支持在区块链中存储数据。OP_RETURN的改变创建了一个可证明可修剪的输出，以避免一些已经部署的数据存储方案将任意数据（如图像）存储为永远无法花费的交易输出，从而导致比特币的UTXO数据库膨胀。
