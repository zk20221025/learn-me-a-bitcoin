...
P2SH是在**2012年4月**引入的标准脚本。它使用户能够将比特币锁定到脚本的哈希值上，然后在解锁这些比特币时提供原始脚本。

它使用户能够**创建自己的自定义“赎回脚本”，并且可以方便地与其他人分享**。

>BIP 16：[Pay To Script Hash](https://github.com/bitcoin/bips/blob/master/bip-0016.mediawiki)（2012年1月3日）

## P2SH的工作原理是什么？
锁定脚本包含另一个锁定脚本的哈希值（“**脚本哈希**”），这个哈希值被**HASH160**和**EQUAL**操作码包围：
| scriptPubKey|OP_HASH160 748284390f9e263a4b766a75d0633c50426eb875 OP_EQUAL|
|---|---|

解锁脚本则包含你原始的自定义锁定脚本（“**赎回脚本**”），并在其前面添加解锁所需的数据或操作码：

![P2SH-2.png](img/P2SH-2.png)

因此在上述解锁脚本中，**赎回脚本**本身就是一个[P2MS](../P2MS/P2MS.md)锁定脚本，前面的签名就是解锁它所需的。
...
1. **标准执行**——首先对赎回脚本进行哈希处理，然后检查它是否等于锁定脚本中的**脚本哈希值**。
...
通常情况下，这就是脚本执行结束的地方。然而，由于比特币改进提案[BIP 16](https://github.com/bitcoin/bips/blob/master/bip-0016.mediawiki)对比特币所做出的更改，我们进入了P2SH脚本执行的第二步...
...
在赎回脚本被反序列化之后，整个脚本就像任何标准脚本一样运行。由于上面的示例中**赎回脚本**包含[P2MS](../P2MS/P2MS.md)脚本，因此整个脚本的运行方式如下：
<center>
...
在P2SH出现之前，如果想要在你的比特币上放置一个复杂的锁定脚本（例如[P2MS](../P2MS/P2MS.md)），必须将**整个锁定脚本**提供给“发送”这些比特币给你的人：
...
这将增加交易数据的大小，导致发送方需要支付更多的[交易手续费](../../Transaction/Fees/Fees.md)来容纳你的脚本。

但是，使用P2SH，他们可以使用你的脚本的哈希值来创建锁定，这比完整脚本要小得多。

![P2SH-9.png](img/P2SH-9%20(1).png)

因此，**大型锁定脚本的负担不再由发送者承担**（而是转移到接收者身上）。避免了发送者只愿意发送比特币到最简单和最便宜的锁定脚本的情况。（例如[P2PKH](../P2PKH/P2PKH.md)）

### 2. 我们可以使用地址来表示P2SH锁定脚本。
由于每个P2SH锁定脚本都具有统一的结构（得益于固定大小的[哈希值](../../Other/Hash%20Function/Hash%20Function.md)），我们可以为它设计一个标准[地址](../../Keys/Address/Address.md)格式（类似于[P2PKH](../P2PKH/P2PKH.md)地址格式）。


![P2SH-10.png](img/P2SH-10%20(1).png)

P2SH地址以**3**开头（因为在转换为[base58](../../Keys/Base58/Base58.md)之前P2SH地址添加了**0x05**前缀）。

不必传递实际的P2SH锁定脚本，而是可以将其转换为地址并缩短它（再次缩短），从而更容易分享。

![P2SH-11.png](img/P2SH-11%20(1).png)

记住，地址包含[校验和](../../Keys/Checksum/Checksum.md)，并使用[base58](../../Keys/Base58/Base58.md)字符，因此它们比**原始脚本哈希**更适合分享。

### 3. P2SH具有更高的隐私性。

由于P2SH锁定脚本仅包含另一个锁定脚本的哈希值，因此仅仅通过查看哈希值，不可能知道该哈希值来自什么类型的锁定脚本：

![P2SH-12.png](img/P2SH-12%20(1).png)

**脚本哈希**可以是任何类型的脚本的[哈希值](../../Other/Hash%20Function/Hash%20Function.md)。

因此，没有人会知道一个输出是否属于一个人的公钥，还是属于多个人。

当然，除非有人花费它并揭示赎回脚本。

### 4.更小的UTXO集。
为了提高比特币程序的运行速度，所有未花费的交易输出（[UTXOs](../../Transaction/UTXO/)）都存储在计算机的RAM中。
...
如果无法将整个UTXO集放入RAM中，节点验证交易的速度就会变慢。
...
自**Bitcoin 0.10.0**以来，你不再受限于仅使用标准脚本：

>对于P2SH赎回脚本，IsStandard（）规则几乎完全被移除，允许应用程序使用任何有效的脚本类型，例如“n-of-m OR y”，哈希锁定的预言机地址等。虽然比特币协议始终支持这些类型的脚本，但在主网上实际使用它们非常不方便，因为标准比特币核心节点不会将它们转发给矿工，大多数矿工也不会将它们包含在他们挖掘的区块中。-[比特币0.10.0发布说明](https://bitcoin.org/en/release/v0.10.0#standard-script-rules-relaxed-for-p2sh-addresses)

所以，如果你想在各种**操作码**中寻找乐趣，P2SH的**赎回脚本**就是你可以实现的地方。
...
而完整的2-of-3多重签名脚本使用**P2SH**在区块链中占用**278个字节**：
<center>

![P2SH-15.png](img/P2SH-15%20(1).png)</center>

因此额外的**25字节**是由于将比特币锁定到脚本哈希的步骤所导致的。

然而，对于能够为自定义锁定脚本创建[地址](../../Keys/Address/Address.md)，这额外的25个字节是值得的。
