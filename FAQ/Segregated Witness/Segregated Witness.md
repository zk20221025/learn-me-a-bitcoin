...
此提议的主要目的是为了解决**交易可延展性**问题，这是指一种攻击者可以改变比特币交易的唯一标识符（即交易ID）的情况。然而，这种改变也带来了其他好处，比如**增加了一个区块可以容纳的交易数量**。
...
所以从本质上说，你已经将交易的“验证”部分（解锁代码）与“有效”部分（比特币的移动）中分离开来。

如果你将这个验证代码称为见证数据（正如密码学家可能会这样称呼），你可以说你已经“隔离了见证”。这就是"隔离见证"名称的由来。
...
所以，区块大小限制乘以4，得到新的区块权重限制。然后，交易中的每个字节也乘以4，得出[交易权重](../../Technical/Transaction/Weight/Weight.md)。然而，你只需要将见证数据的字节乘以1，这为你节省了它在一个区块中占用空间的75%。

所以你可以说，解锁数据占用的空间只有原来的1/4，这为区块中的交易释放了更多的空间。

>这是对区块大小的增加吗？  
是的，现在每个区块的大小可以超过1,000,000字节（1MB）。  
虽然区块大小限制并没有具体增加多少，但解锁数据的空间占用得到了折扣，意味着区块可以包含的数据量会超过当前的1MB限制。

>隔离见证的“区块大小增加”有多少？  
大约是**1.8MB**。  
**我是从哪里得到这个数字的？**  
目前，一个传统的交易区块大约**包含60%的解锁数据**。[^1]  
所以，如果计算一个由“传统”数量的解锁数据组成的1MB区块的权重，会得到：
```
...
>仅仅因为区块大小限制从1,000,000字节改变到4,000,000权重单位，并不代表隔离见证实际上将区块大小增加到4MB，因为传统的区块不会全部填充见证数据（每字节1个权重单位）。  
相反，交易将包含正常数据（4个权重单位）和见证数据（1个权重单位）。所以“区块大小增加”将根据其中的解锁数据和其他数据的比例而变化。
...
例如，其中一个规则是每个区块等大小必须1MB或更小。

因此，如果你想做出这些改变，**你需要让网络上的所有人升级他们的软件**（并且同意这些改变）。

否则，网络会出现分裂，新的节点会构建新的区块链，旧的节点会继续构建旧的区块链。
...
采用隔离见证的方法，**交易和区块仍然遵循比特币网络的当前规则**，所以所有节点都会认为隔离见证区块是有效的。因此，“旧”节点将接受这些“新”区块，并将它们添加到他们的区块链中。
...
总的来说，隔离见证看起来像是一种曲线救国的方法来修复交易可延展性和增加区块容量，这种方法避免了试图让每个人升级到新软件（否则就会落后）的问题。
...
95%的阈值是在两个重新定向区块之间计算的。如果超过95%，软分叉将在下一个重新定向区块（大约2周后的2016个区块）被激活。
